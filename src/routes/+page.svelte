<script>
  ...
  async function exportImage() {
    ...
    try {
      ...
      const img = new Image();
      // Set crossOrigin for external images to avoid CORS issues when reading dimensions
      ...
      const displayedImageRect = previewImage.getBoundingClientRect();
      const scale = Math.min(
        originalWidth / displayedImageRect.width,
        originalHeight / displayedImageRect.height
      );

      const containerStyles = window.getComputedStyle(previewBg);
      const padding = parseFloat(containerStyles.padding) || 0;
      const scaledPadding = padding * scale;

      const exportWidth = originalWidth + scaledPadding * 2;
      const exportHeight = originalHeight + scaledPadding * 2;

      console.log("Image source dimensions:", originalWidth, originalHeight);
      console.log("Image export dimensions:", exportWidth, exportHeight);

      previewBg.classList.add("exporting");
      previewBg.style.width = `${exportWidth}px`;
      previewBg.style.height = `${exportHeight}px`;
      previewImage.style.width = `${originalWidth}px`;
      previewImage.style.height = `${originalHeight}px`;

      const dataUrl = await domToPng(previewBg, {
      ...
      previewBg.classList.remove("exporting");
      previewBg.style.width = "";
      previewBg.style.height = "";
...
>
  ...
  >
    ...
    <div class="flex justify-center mt-1">
      <div
        class="text-sm text-gray-500 font-normal dark:text-gray-400 text-center"
        style="line-height: 1.25rem;"
      >
        Upload an image, create a border and download.<br /><span
          class="simple-accent">Simple.</span
        >
      </div>
    </div>
    ...
    <section class="flex justify-center mt-2">
      <div class="rounded-2xl p-2 sm:p-3 max-w-2xl w-full">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 items-start">
          <div
            class="flex flex-col items-center gap-2 justify-center bg-gray-100/50 dark:bg-gray-800/40 border-none border-gray-300 dark:border-gray-600 rounded-lg p-2 sm:p-3 min-h-[120px]"
          >
          ...
          <div
            class="flex flex-col items-center gap-1 justify-center bg-gray-100/50 dark:bg-gray-800/40 border-none border-gray-300 dark:border-gray-600 rounded-lg p-2 sm:p-3"
          >
            <div
...
<style>
  ...
  :global(.dark) .simple-accent::after {
